...
function [hMeanPlots] = shaded_plot(x_data, y_data, stats)
...
for i_curve = 1 : n_curve
    ...
    plot(x_data, curve_min, 'color', colors{i_curve});
    hold on
    plot(x_data, curve_max, 'color', colors{i_curve})
    
    % Fill in shaded region 
    x_data_2 = [x_data, fliplr(x_data)];
    inBetween = [curve_min, fliplr(curve_max)];
    fill(x_data_2, inBetween, colors{i_curve}, 'FaceAlpha', 0.1, 'EdgeAlpha', 0);
